...
from ..serializers import PendingRequestListSerializer, PendingRequestCreateSerializer
...
class PendingRequestList(ListAPIView):
    ...
    def get_queryset(self):
        ...
        if not (
            Room.objects.filter(Q(teachers=user_id) | Q(class_representatives=user_id))
        ):
...
class PendingRequestCreate(CreateAPIView):
    ...
    def create(self, request, *args, **kwargs):
        ...
        if Room.objects.filter(Q(teachers=user_id) | Q(students=user_id), id=room_pk):
